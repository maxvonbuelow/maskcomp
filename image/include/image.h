...
struct imgdta {
	imgdta(std::size_t width, std::size_t height = 1, std::size_t channels = 1, std::size_t bd = 8, bool is_float = false) : w(width), h(height), nc(channels), bd(bd), _data(w * h * nc * (bd + 7) / 8), flt(is_float)
	{}
	imgdta() : imgdta(0, 0)
	{}
	...
	void resize(std::size_t width, std::size_t height = 1, std::size_t channels = 1, std::size_t _bd = 8)
	{
		w = width;
		h = height;
		nc = channels;
		bd = _bd;
		_data.resize(w * h * nc * bps());
	}
...
struct image : imgdta {
	...
	{}
	image(std::size_t width, std::size_t height = 1, std::size_t channels = 1, std::size_t bd = sizeof(T) * 8) : imgdta(width, height, channels, bd, std::is_floating_point<T>::value)
	{}
	image() : image(0, 0)
	{}
	void resize(std::size_t width, std::size_t height = 1, std::size_t channels = 1, uint32_t bitdepth = sizeof(T) * 8)
	{
		imgdta::resize(width, height, channels, bitdepth);
	}
